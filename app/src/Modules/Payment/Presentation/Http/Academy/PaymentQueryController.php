...
use App\Modules\Payment\Application\Handler\ShowGuardianPaymentHistoryHandler;
...
{
    public function __construct(private readonly Security $security, private readonly ShowPlayerDebtHandler $debtHandler, private readonly ShowPaymentHistoryHandler $historyHandler, private readonly ShowGuardianPaymentHistoryHandler $guardianHistoryHandler, private readonly TenantContext $tenantContext) {}
    ...
    #[Route('/guardians/{guardianId}/history', methods:['GET'])]
    public function guardianHistory(string $guardianId): JsonResponse
    {
        $items = ($this->guardianHistoryHandler)(new ShowGuardianPaymentHistoryQuery($this->tenantContext->requireAcademyId(), $guardianId));
        return new JsonResponse(['data'=>array_map(static fn($item) => $item->toArray(), $items),'meta'=>new \stdClass()]);
    }
}
