...
final readonly class PlayerDocumentUploadValidator implements Contract
{
    ...
    public function validate(UploadedFile $file): array
    {
        ...
        if (false === $size || $size <= 0 || $size > self::MAX_SIZE) {
            throw new \InvalidArgumentException('El archivo debe pesar entre 1 byte y 3 MB.');
        }
        if (!isset(self::MIME_EXTENSIONS[$mime])) {
            throw new \InvalidArgumentException('El formato del archivo no está soportado.');
        }
