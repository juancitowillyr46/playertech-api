...
final readonly class ReplacePlayerDocumentHandler
{
    ...
    {
        $academy = new AcademyId($command->academyId); $document = $this->repository->findActiveById($academy, new PlayerId($command->playerId), new PlayerDocumentId($command->documentId)) ?? throw new \RuntimeException('Documento no encontrado.');
        $file = $this->validator->validate($command->file); $file['storageName'] = $this->storage->store($command->file, $file['extension']);
        $old = $document->replace(DocumentType::fromInput($command->documentType), $file, $command->observations, $command->actorId);
        try { $this->repository->save($document); } catch (\Throwable $e) { $this->storage->delete($file['storageName']); throw $e; }
        $this->storage->delete($old); return PlayerDocumentResponse::fromDocument($document);
    }
}
