...
final readonly class ReactivateAcademyHandler
{
    ...
    public function __invoke(ReactivateAcademyCommand $command): AcademyResponse
    {
        $academy = $this->requireAcademy($command->academyId);

        $academy->reactivate($command->actorId);
