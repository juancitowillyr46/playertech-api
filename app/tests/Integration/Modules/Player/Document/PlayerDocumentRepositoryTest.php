...
final class PlayerDocumentRepositoryTest extends KernelTestCase
{
    public function testItListsOnlyActiveDocumentsForThePlayerAndAcademy(): void
    {
        ...
        $em->persist($player); $em->persist($otherPlayer); $em->flush();
        ...
        $deleted = PlayerDocument::create(PlayerDocumentId::generate(), $academy, $player->id(), DocumentType::CE, $file('ce.pdf'), null, 'actor'); $deleted->delete('actor');
        ...
        $em->persist($active); $em->persist($deleted); $em->persist($crossTenant); $em->flush();
        ...
        self::assertSame(1, $result['total']); self::assertSame($active->id()->value(), $result['items'][0]->id()->value());
    }
}
