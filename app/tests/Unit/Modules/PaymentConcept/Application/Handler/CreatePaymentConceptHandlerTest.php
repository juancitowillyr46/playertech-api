...
final class CreatePaymentConceptHandlerTest extends TestCase
{
    public function testItCreatesPaymentConcept(): void
    {
        ...
        $handler = new CreatePaymentConceptHandler($repository);
        $response = $handler(new CreatePaymentConceptCommand('019eec93-9a11-7432-bd04-52306b2b3d8e','019eec93-9a11-7432-bd04-52306b2b3d8f', new CreatePaymentConceptInput('matricula','Matrícula','Cobro de ingreso')));
        self::assertSame('MATRICULA', $response->toArray()['code']);
        self::assertCount(1, $repository->items);
    }
    public function testItRejectsDuplicateCode(): void
    {
        $repository = new InMemoryPaymentConceptRepository();
        $handler = new CreatePaymentConceptHandler($repository);
        $command = new CreatePaymentConceptCommand('019eec93-9a11-7432-bd04-52306b2b3d8e','019eec93-9a11-7432-bd04-52306b2b3d8f', new CreatePaymentConceptInput('matricula','Matrícula','Cobro de ingreso'));
        $handler($command);
