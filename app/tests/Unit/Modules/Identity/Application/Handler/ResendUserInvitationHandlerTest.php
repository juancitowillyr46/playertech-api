...
final class ResendUserInvitationHandlerTest extends TestCase
{
    public function testItResendsInvitationOnlyForPendingUsers(): void
    {
        ...
        $handler = new ResendUserInvitationHandler($entityManager, $messageBus, $passwordHasher, 'http://localhost:4200');
        $response = $handler(new ResendUserInvitationCommand('actor-id', '019eec93-9a11-7432-bd04-52306b2b3d8e', '019eec93-9a11-7432-bd04-52306b2b3d8f', 'INVITATION'));

        self::assertSame(AccountUser::STATUS_PENDING_ACTIVATION, $response->toArray()['status']);
    }

    public function testItResendsGeneratedPasswordWhenRequested(): void
    {
        $user = new AccountUser();
        $user->setEmail('juan@test.local');
        $user->setFullName('Juan Perez');
        ...
        $handler = new ResendUserInvitationHandler($entityManager, $messageBus, $passwordHasher, 'http://localhost:4200');
        $response = $handler(new ResendUserInvitationCommand('actor-id', '019eec93-9a11-7432-bd04-52306b2b3d8e', '019eec93-9a11-7432-bd04-52306b2b3d8f', 'PASSWORD'));
