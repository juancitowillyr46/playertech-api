...
final class CreatePlayerDocumentHandlerTest extends TestCase
{
    ...
    {
        $academyId = AcademyId::generate(); $player = Player::create(PlayerId::generate(), $academyId, 'CC', 'Ana', 'Rojas', new \DateTimeImmutable('2010-01-01'), '100', null, null, null, null, null, null, null, null, AuditTrail::create('actor'));
        $players = $this->createMock(PlayerRepository::class); $players->method('findById')->willReturn($player);
        $storage = $this->createMock(PlayerDocumentStorage::class); $storage->expects(self::never())->method('store');
        $validator = $this->createMock(PlayerDocumentUploadValidator::class); $validator->expects(self::never())->method('validate');
