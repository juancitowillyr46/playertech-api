...
final class PlayerDocumentControllerTest extends TestDatabaseKernel
{
    ...
    protected function setUp(): void
    {
        ...
        $user->setEmail('documents-admin@test.local');
        ...
        $user->setFullName('Document Admin');
        $this->entityManager->persist($academy);
        $this->entityManager->persist($player);
        $this->entityManager->persist($otherAcademy);
        $this->entityManager->persist($otherPlayer);
        $this->entityManager->persist($user);
        $this->entityManager->flush();
        $this->playerId = $player->id()->value();
        $this->otherPlayerId = $otherPlayer->id()->value();
    ...
    public function testItUploadsListsViewsDownloadsReplacesAndDeletesDocument(): void
    {
        ...
        $documentId = json_decode($first->getContent(), true, 512, JSON_THROW_ON_ERROR)['data']['id'];
        $crossTenant = $this->request('/api/v1/academy/players/' . $this->otherPlayerId . '/documents', 'GET');
        self::assertSame(404, $crossTenant->getStatusCode());
        $list = $this->request('/api/v1/academy/players/' . $this->playerId . '/documents', 'GET');
        self::assertSame(200, $list->getStatusCode(), $list->getContent());
        self::assertCount(1, json_decode($list->getContent(), true, 512, JSON_THROW_ON_ERROR)['data']);
        $view = $this->request('/api/v1/academy/players/' . $this->playerId . '/documents/' . $documentId, 'GET');
        self::assertSame(200, $view->getStatusCode());
        self::assertStringContainsString('inline', (string) $view->headers->get('Content-Disposition'));
        $download = $this->request('/api/v1/academy/players/' . $this->playerId . '/documents/' . $documentId . '/download', 'GET');
        ...
        $replacement = $this->upload('second.pdf', '%PDF-1.4 second', 'PUT', $documentId);
        self::assertSame(200, $replacement->getStatusCode());
        self::assertSame($documentId, json_decode($replacement->getContent(), true, 512, JSON_THROW_ON_ERROR)['data']['id']);
        $delete = $this->request('/api/v1/academy/players/' . $this->playerId . '/documents/' . $documentId, 'DELETE');
        self::assertSame(204, $delete->getStatusCode());
        $empty = $this->request('/api/v1/academy/players/' . $this->playerId . '/documents', 'GET');
        self::assertCount(0, json_decode($empty->getContent(), true, 512, JSON_THROW_ON_ERROR)['data']);
    }
    ...
    {
        $path = tempnam(sys_get_temp_dir(), 'player-document');
        file_put_contents($path, $contents);
        $file = new UploadedFile($path, $name, 'application/pdf', null, true);
        $suffix = null === $documentId ? '' : '/' . $documentId;
        $request = Request::create('/api/v1/academy/players/' . $this->playerId . '/documents' . $suffix, $method, parameters: ['documentType' => 'CC', 'observations' => 'Identidad'], files: ['file' => $file], server: ['HTTP_AUTHORIZATION' => 'Bearer ' . $this->token]);
