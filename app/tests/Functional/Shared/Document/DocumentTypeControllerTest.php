...
namespace App\Tests\Functional\Shared\Document;
...
final class DocumentTypeControllerTest extends TestDatabaseKernel
{
    ...
    public function testItReturnsTheCatalogForAnAuthenticatedTenantUser(): void
    {
        $response = self::$kernel->handle(Request::create(
            '/api/v1/academy/document-types/options',
            'GET',
        ...
        self::assertCount(6, $body['data']);
        self::assertSame('CE', $body['data'][0]['value']);
        self::assertSame([], $body['meta']);
    }

    public function testItRejectsAnUnauthenticatedRequest(): void
    {
        $response = self::$kernel->handle(Request::create('/api/v1/academy/document-types/options', 'GET'));
