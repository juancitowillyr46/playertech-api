...
6. **Given** a payload with `categoryId` at create time, **When** an admin registers a player, **Then** the player keeps that category and the detail response exposes `categoryId`.
