...
* El selector contextual de equipos por jugador queda definido como `GET /api/v1/academy/players/{playerId}/teams/options?q={texto}` y excluye equipos ya asignados activamente a ese jugador.
...
* `HU-003` de `EP-007-player-management` quedó implementada y validada en runtime con `GET /api/v1/academy/players/{playerId}`; el response incluye `categoryName`, `documentTypeName`, `nationalityName`, `genderName`, `dominantFootName`, `phoneSingle`, `legalGuardianMain` y `teamMain` como resúmenes del contexto principal del jugador.
...
* El listado de `LegalGuardian` ahora normaliza `sort` con aliases seguros (`created_at`, `document_number`, `first_name`, `last_name`, `status`) y filtra por `documentNumber`, `documentType`, `firstName`, `lastName` y `fullName` con el mismo patrón de `Player`, incluyendo búsqueda case-insensitive y accent-insensitive.
