...
La frontera que conviene respetar es esta:

- `Membership` administra la matrícula y puede disparar cargos automáticos.
- `Charge` modela la deuda concreta.
- `Payment` sólo aplica el recaudo sobre cargos existentes.
- `PaymentAllocation` distribuye el pago entre cargos.
- `PaymentConcept` sigue siendo el catálogo reusable de motivos de cobro.
