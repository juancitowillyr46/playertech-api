...
Este documento resume el contrato vigente del dominio `Academy`, con foco en el contrato visible para frontend y en los flujos operativos del tenant.
...
- `GET /api/v1/academy/context` expone el contexto operativo del usuario autenticado.
- `GET /api/v1/academy/me` y `PUT /api/v1/academy/me` son la vía principal de autogestión del tenant.
- `POST /api/v1/academy/me/shield` acepta `multipart/form-data` con campo `shield`.
- `DELETE /api/v1/academy/me/shield` elimina la imagen institucional y responde `204`.
- `GET /api/v1/academy/me/tax-profile` y `PUT /api/v1/academy/me/tax-profile` cubren el perfil fiscal operativo.
- `registrationSource` diferencia academias creadas por `signup` y por `platform`.
